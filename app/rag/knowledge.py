...
def _split_by_paragraphs(text: str) -> list[str]:
    ...
    for para in paragraphs:
        if len(current) + len(para) > _CHUNK_SIZE and current:
            ...
            words = current.split()
            overlap_words = words[-_CHUNK_OVERLAP // 5 :]
...
def _split_code_by_functions(content: str, language: str) -> list[str]:
    ...
    try:
        ...
        _collect_boundaries(tree.root_node, boundaries)
...
def _collect_boundaries(node, boundaries: list[int]) -> None:
    func_types = {
        "function_definition",
...
async def ingest_document(
    ...
    """Split document into chunks, embed them, and store them."""
    ...
    for chunk_obj in chunk_objs:
        embedding = json.loads(chunk_obj.embedding_json or "[]")
        ...
        await db.execute(
            ...
            {"embedding": vec_str, "chunk_id": str(chunk_obj.id)},
        )
